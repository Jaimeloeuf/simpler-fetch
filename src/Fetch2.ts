 ...
 */
export class Fetch2<ResponseType, ErrorType> {
  ...
  /**
   */
  readonly #responseParser: ResponseParser<ResponseType>;

  /**
   * Optional cos if unspecified will parse the same way as the original data type
   */
  readonly #optionalErrorParser: ResponseParser<ErrorType>;
  ...
  constructor(
    ...
    errorParser: (res: Response) => Promise<ErrorType>,
  ...
  validateWith<T extends ResponseType = ResponseType>(
    responseValidator: Validator<T>
  ): Fetch2<T, ErrorType> {
    this.#optionalResponseValidator = responseValidator;

    return this as unknown as Fetch2<T, ErrorType>;
  }
  ...
  call<ExpectedResponseType extends ResponseType = ResponseType>() {
    return safe(async () => {
      ...
      if (res.ok) {
        ...
        // const data: ExpectedResponseType = await this.#responseParser(res);
      ...
      // If API call failed
      // still parse, but cast to the other type
      // skip validation since the validation is just for the success Type
      // and still return ApiResonse<> but use FailedType as the generic

      return {
        ok: res.ok,
        status: res.status,
        headers: res.headers,
        data: (await this.#optionalErrorParser(res)) as ErrorType,
