 ...
 * rarely used, therefore these are the only 2 methods which do not have wrapper
 * methods to easily create `Fetch` instances from `Builder` instance. However this
 * can be solved by library users simply by using the `HTTP` method on `Builder`.
 *
